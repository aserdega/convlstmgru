...
lstm_dims = [128,128,128]

#----------create some dirs---------

root_log_dir = './results_gru_' + str(lstm_dims)[1:-1].replace(', ','x')
...
def save_model(step, train_bce, test_bce):
    new_dir =  chkpnt_dir + "/{0:0>5}iter | test bce:{1:.4f} | train bce:{2:.4f}".format(step, test_bce, train_bce)
    os.makedirs(new_dir)
...
for e in range(100):
    for _, batch in enumerate(dloader):
        ...
        prevf_enc = cnn_encoder(prevf_raw).view(b_size,10,hidden_dim,16,16)

        nextf_enc = cnn_encoder(seqs[:,10:,:,:,:].contiguous().view(-1,1,64,64).cuda()).view(b_size,10,hidden_dim,16,16)
        ...
        decoder_out_rs  = final_decoder_out.view(-1,hidden_dim,16,16)
